...
const KEYWORDS = new Set([
  ...
  'case', 'break', 'continue', 'new', 'delete', 'typeof', 'instanceof', 'in',
  'of', 'async', 'await', 'try', 'catch', 'finally', 'throw', 'yield',
  'static', 'public', 'private', 'protected', 'abstract', 'interface', 'type',
  'enum', 'namespace', 'declare', 'module', 'implements', 'super', 'this',
  'true', 'false', 'null', 'undefined', 'void', 'never', 'as', 'satisfies',
  // Python
  'def', 'lambda', 'with', 'pass', 'global', 'nonlocal', 'assert', 'del',
  'raise', 'not', 'and', 'or', 'is', 'elif', 'except', 'print',
  // Rust
  'fn', 'mut', 'pub', 'use', 'mod', 'struct', 'impl', 'trait', 'where',
  'match', 'Self', 'self', 'crate', 'move', 'ref', 'unsafe', 'extern',
  'dyn', 'loop', 'box',
  // Java
  'package', 'throws', 'final', 'synchronized', 'transient', 'volatile',
  'native', 'strictfp', 'goto', 'byte', 'short', 'int', 'long', 'float',
...
export function tokenizeFile(tree: SyntaxTree): Map<number, Token[]> {
  ...
  sourceLines.forEach((lineText, lineIdx) => {
    ...
    map.set(lineIdx, tokens.filter(t => t.text.length > 0));
  });
